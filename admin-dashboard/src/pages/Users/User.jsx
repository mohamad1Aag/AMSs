...
export default function User() {
  ...
  const fetchUsers = async () => {
    try {
      const response = await axios.get("https://my-backend-dgp2.onrender.com/api/users", {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`https://my-backend-dgp2.onrender.com/api/users/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handlePointChange = (e) => setPointToAdd(Number(e.target.value));

  const handlePointSubmit = async (e) => {
    e.preventDefault();
    if (pointToAdd <= 0) return alert("يجب إدخال قيمة نقاط صحيحة أكبر من صفر");
    ...
    try {
      ...
      if (res.status === 200) {
        setUsers(users.map((u) => (u._id === editingUserId ? { ...u, point: newPoint } : u)));
  ...
  return (
    <>
      {!sidebarOpen && (
        <button className="fixed top-4 left-4 z-50 bg-white p-2 rounded-md shadow" onClick={toggleSidebar}>
      ...
      <div className="min-h-screen bg-gradient-to-r from-purple-800 via-pink-600 to-yellow-100 p-6">
        <h2 className="text-3xl text-white font-bold mb-6 text-center">إدارة المستخدمين</h2>
            ...
            <td className="p-3">
              <span
                className={`px-3 py-1 rounded-full text-xs ${
                  user.status === "مفعل" ? "bg-green-500 text-white" : "bg-red-500 text-white"
                }`}
              >
...
<div className="md:hidden space-y-4" dir="rtl">
  {filteredUsers.length > 0 ? (
    filteredUsers.map((user, index) => (
      <div key={user._id} className="bg-white rounded-lg shadow p-4 space-y-2 text-sm text-right">
        <p><strong>الرقم:</strong> {index + 1}</p>
        <p><strong>الاسم:</strong> {user.name}</p>
        <p><strong>البريد:</strong> {user.email}</p>
        <p><strong>الهاتف:</strong> {user.phone || "-"}</p>
        <p><strong>النقاط:</strong> {user.point || 0}</p>
        <p><strong>الدور:</strong> {user.role || "غير محدد"}</p>
        <p>
          <strong>الحالة:</strong>{" "}
          <span className={`px-2 py-1 rounded text-white text-xs ${
            user.status === "مفعل" ? "bg-green-500" : "bg-red-500"
          }`}>
            {user.status || "غير معروف"}
          </span>
        </p>
    ...
    <div className="text-center text-white">لا توجد نتائج</div>
  )}
</div>
        ...
        {editingUserId && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white p-6 rounded-lg shadow-xl w-80">
              <h3 className="text-lg font-semibold mb-4">زيادة النقاط للمستخدم</h3>
